          ...
          <div className='divide-y'>
            {cartData.map((item, index) => {
              ...
              const getPriceBySize = () => {
                ...
                if (item.size === '10ML') return (basePrice / 10).toFixed(2);
                if (item.size === '50ML') return (basePrice / 2).toFixed(2);
                // Default or 100ML
                return basePrice;
              };
              ...
              return (
                <div key={index} className='py-6 grid grid-cols-1 sm:grid-cols-2 gap-4 items-center text-gray-700'>
                  <div className='flex items-start gap-3 mt-2'>
                    <img className='w-20 sm:w-24 rounded-md object-cover' src={productData.image[0]} alt={productData.name} />
                    <div>
                      <p className='text-sm sm:text-lg font-medium'>{productData.name}</p>
                      <div className='flex items-center gap-3 mt-2'>
                        <p className='text-gray-800 font-semibold'>{currency}{getPriceBySize()}</p>
